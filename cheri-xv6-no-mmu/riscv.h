...
{
  uintptr_t  x;
  asm volatile("cspecialr %0, mepcc" : "=C" (x) );
  return x;
}
...
static inline void
w_mscratch(uintptr_t  x)
{
  // asm volatile("csrw mscratch, %0" : : "r" (x));
  asm volatile("cspecialw mscratchc, %0" : : "C" (x));
}

// Default Data Capability (DDC) access functions
static inline void* __capability
r_ddc()
{
  void* __capability x;
  asm volatile("cspecialr %0, ddc" : "=C" (x));
  return x;
}

static inline void
w_ddc(void* __capability x)
{
  asm volatile("cspecialw ddc, %0" : : "C" (x));
}
...
#define PERMSHIFT 10 // bits of permissions in a PTE
#define PTSIZE (512*3) // Size of the page table in bytes
#define NPROC 8
