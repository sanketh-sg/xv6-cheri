...
void start(){
    ...
    // Set mscratch to point to user physical memory for context switching
    // w_mscratch((uintptr_t)pcb[0].phybase);

    // Set up user DDC (Default Data Capability) for user space access
    // Create a capability that covers the entire user virtual address space
    // Use the original DDC but set address to 0 and bounds to cover full user space
    // void* __capability user_ddc = cheri_bounds_set(
    //     cheri_address_set(cheri_ddc_get(), 0x0),
    //     0x200000  // 2MB user space
    // );
    // w_ddc(user_ddc);
...
void enable_paging(void) {
    for (int i = 0; i < NPROC; i++) {
        ...
        pcb[i].sp = cheri_bounds_set(
            cheri_address_set(cheri_ddc_get(), 0x1ffff8),
            0x1000
        );
